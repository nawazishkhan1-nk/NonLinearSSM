...
class MAF(nn.Module):
    ...
    def __init__(self, dim, parity, device, net_class=ARMLP, nh=24):
        super().__init__()
        self.dim = dim
        self.device = device
        self.net = net_class(dim, dim*2, nh, device)
        self.parity = parity
...
class NormalizingFlowModel(nn.Module):
    ...
    def __init__(self,flows, device, mean, cov):
        super().__init__()
        self.mean = mean
        self.cov = cov
        self.device = device
    ...
    @property
    def base_dist(self):
        return MVN(self.mean, self.cov, validate_args=False)
    def forward(self, x):
        if len(x.size()) == 1:
            x = x[None, :]
        zs, log_det = self.flow.forward(x)
        prior_logprob = self.base_dist.log_prob(zs[-1]).view(x.size(0), -1).sum(1)
    ...
    def log_prob(self, x):
        zs, prior_log_prob, log_det = self.forward(x)

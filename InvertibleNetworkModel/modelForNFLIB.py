...
from nflib.flows import (MAF, AffineHalfFlow, NormalizingFlowModel)
    ...
    for i, data in enumerate(dataloader):
        ...
        if epoch % args.log_interval == 0:
            ...
            if args.plot_projections:
                ...
                # print(f'z0_ten shape is {z0_ten.shape} {z0.shape}')
                z0 = z0.reshape((-1, 3))
                ax.scatter3D(z0[:, 0], z0[: , 1], z0[:, 2], color = "green")
...
class InvertibleNetwork:
    ...
    def initialize_model(self):
        args = self.params
        if args.model == 'maf':
            flows = [MAF(dim=args.input_size, parity=i%2, device=args.device, nh=args.hidden_size) for i in range(args.n_blocks)]
            self.model = NormalizingFlowModel(flows, self.device, mean=self.prior_mean, cov=self.prior_cov)
        elif args.model =='realnvp':
            flows = [AffineHalfFlow(dim=args.input_size, parity=i%2, nh=args.hidden_size) for i in range(args.n_blocks)]
            self.model = NormalizingFlowModel(flows, self.device, mean=self.prior_mean, cov=self.prior_cov)
        else:
